...
import {
  ...
  updateStatusBookingDataRequest,
            ...
            {oneBooking.map((e, id) => {
              return (
                <StyledTableRow key={id}>
                  ...
                  <StyledTableCell>{e.userID.name}</StyledTableCell>
                  ...
                  <StyledTableCell
                    align="center"
                    onClick={() => {
                      const text = "Do you want to Approve this User Booking?";
                      const id1 = JSON.parse(localStorage.getItem("ID"));
                      if (confirm(text) === true) {
                        const id = e._id;
                        dispatch(updateStatusBookingDataRequest(id));
                        alert("User's Booking Approved.");
                        dispatch(getBookingDataRequest(id1));
                      }
                    }}
                  >
                    <Button variant="contained">{e.status}</Button>
                  </StyledTableCell>
                  <StyledTableCell align="center">
                    <Button
                      variant="contained"
                  ...
                  <StyledTableCell align="center">
                    <Button
                      ...
                      onClick={() => {
                        const text =
                          "Do you want to delete this User's Booking?";
                        if (confirm(text) === true) {
                          const id = e._id;
